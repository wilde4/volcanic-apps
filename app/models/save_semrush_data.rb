class SaveSemrushData
  def self.save_data(app_setting_id)
    ...
    if app_setting.has_records?
      # organic_report = report.organic(limit: app_setting.keyword_amount, db: app_setting.engine, display_date: Date.today.strftime('%Y%m%%d'), display_sort: 'nq_desc')
      ...
      organic_report.each do |o|
        ...
        # app_setting.touch(:last_petiton_at)
      ...
      (start_date.to_datetime.to_i .. end_date.to_datetime.to_i).step(request_rate.days) do |date|
        date = Time.at(date)
        if date <= Date.today
          # organic_report = report.organic(limit: app_setting.keyword_amount, db: app_setting.engine, display_date: date.strftime('%Y%m%%d'), display_sort: 'nq_desc')
          organic_report = fake_data;
          organic_report.each do |o|
            app_setting.semrush_stats.create(
              dataset_id: app_setting.dataset_id,
